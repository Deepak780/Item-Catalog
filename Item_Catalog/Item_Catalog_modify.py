...
def editGadgets(gadget_id):
    ...
    if editGadget.user_id not in login_session['user_id']:
        return "<script>function myFunction()alert{('You are not uthorized to edit this category...!')}</script><body onload='myFunction()'></body>"
    if request.method == 'POST':
...
def editGadgetItem(gadget_id,item_id):
    ...
    if editItem.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized to edit. This belongs to....');}</scripts><body onload='myFunction()'>"
    if request.method == 'POST':
        if request.form['name']:
